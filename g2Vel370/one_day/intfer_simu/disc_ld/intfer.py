...
X = dAB[0,0:21]                  # X-position of orbit
Y = dAB[1,0:21]                  # Y-position of orbit
...
def hbt(x,y,X,Y,lam):
    ...
    V_b = (1-l-m) * sp.jv(1, b2)/b2
    V_b += (l+2*m) * np.sqrt(np.pi/2) * sp.jv(3/2, b2)/b2**(3/2)
    V_b -= 2*m * sp.jv(2, b2)/b2**2
    V_b *= I_b 
...
def trans_prior(theta):
    R_aprime, R_bprime, l_prime, m_prime = theta
