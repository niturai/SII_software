...
def hbt(x,y,X,Y,lam,lam1):
    ...
    V_a = (1-l-m) * sp.jv(1, b1)/b1
    V_a += (l+2*m) * np.sqrt(np.pi/2) * sp.jv(3/2, b1)/b1**(3/2)  
    V_a -= 2*m * sp.jv(2, b1)/b1**2
    V_a *= I_a 

    # visibility of source B with ld
    V_2 = (1-l-m) * sp.jv(1, b2)/b2
    V_2 += (l+2*m) * np.sqrt(np.pi/2) * sp.jv(3/2, b2)/b2**(3/2)
    V_2 -= 2*m * sp.jv(2, b2)/b2**2
    V_2 *= I_b 
...
if __name__ == "__main__":
    ...
    def trans_prior(theta):
         R_aprime, R_bprime, R_prime, X_prime, Y_prime, l_prime, m_prime = theta
         ...
         m_min, m_max = 0.5 * ini_m, 1.5 * ini_m     
         ...
         m = m_prime * (m_max - m_min) + m_min
         
         return (R_a, R_b, R1, X, Y, l, m)

     # the liklihood function 
    def lnlikli(ln):
         global R_a, R_b, R1, X, Y, l, m
         R_a, R_b, R1, X, Y, l, m = ln
    ...
    pnames = ('$R_a (ls) $', '$R_b (ls) $', 'R1', 'X', 'Y', 'l', 'm')
    ndim = len(pnames)
    ...
    out = open('data/dres_rl_ldq.pkl','wb')
