...
T1 = 55000                      # Temperature of atmosphere
...
# the model 
def hbt(x,y,X,Y,I_a,I_b,I_1,I_b1):
    r = np.sqrt(x**2 + y**2)
    v = 2*np.pi/D 
    ...
    V_ab = V_a * V_b * np.cos(v * (x*X + y*Y)/lam)

    V = (V_a**2 + V_b**2 + 2 * V_ab)/(I_a + I_b + I_1 - I_b1)**2
    return V
...
tspec += pho_spec(R1, T1, lam1) - pho_spec(R_b, T1, lam1)
...
g = hbt(gX,gY,X,Y,I_a,I_b,I_1,I_b1)               # signal for all baselines
...
np.save("data3/xt",xt)
np.save("data3/yt",yt)
np.save("data3/sig",smdata(0))
np.save("data3/sig_noise",smdata(sigma))
...
def lnlikli(ln):
    ...
    R_a, R_b, R1, X, Y = ln
    g = hbt(gX,gY,X,Y,I_a,I_b,I_1,I_b1)                # Signal for all baseline
    model_data = []
    for i in range(len(base)):
        gi = g[i,:,:]
        xgr = gX[i,0,:]
