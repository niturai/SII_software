...
# calculate the total energy and potential energy
def checks():
...
# read the initial position and velocity of all objects
...
# center of mass of position and velocity
cmx = 0*pos[0]
cmv = 0*vel[0]
for i in range(N_body):
    cmx += mass[i]*pos[i]
...
# position of all objects with leap-frog method
...
np.save('data/dxAB', dxAB)     # x-coordinate of binary BS+WR      
np.save('data/dyAB', dyAB)     # y coordinate of binary BS+WR
np.save('data/dAB', dAB)       # (x,y) coordinate of binary BS+WR
