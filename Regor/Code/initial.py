...
# position of orbit because of (inclination, Omega & omega)
def rotate(x,y,omega,I,Omega):                 
    cs, sn = cos(omega), sin(omega)
    x, y = x*cs - y*sn, x*sn + y*cs
    cs, sn = cos(I), sin(I)
    x,y,z = x, y*cs, y*sn
    Omega += pi/2                         # make north as x-axis
    cs,sn = cos(Omega), sin(Omega)
    x,y,z = x*cs - y*sn, x*sn + y*cs, z
    return np.array((x,y,z))


# define the position and velocity of object in orbit for given anomaly
def posvel(P,a,e,omega,I,Omega,mean_an):
    ...
    pos = a * rotate(x,y,omega,I,Omega)                     # position
    vel = 2*pi * a/P * rotate(vx,vy,omega,I,Omega)          # velocity
    return pos,vel

# define the barycenter of system
def bary(m,x):
    N = len(m)
    cm = 0*m
...
# calculate the initial position of objects 
